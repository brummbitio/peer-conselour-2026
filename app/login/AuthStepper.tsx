...
export default function AuthStepper({
  ...
  }, [isFacultyMenuOpen, isGenderMenuOpen]);
  ...
  const handleLoginSubmit = () => {
    ...
    if (!result.ok) {
      setAuthActionError(result.message || "NIM atau password tidak cocok.");
      return;
    }
  ...
  if (submittedMode) {
    return (
      <section className={isModal ? "auth-page auth-page-modal" : "section site-width auth-page"}>
        <div className={`auth-card glass auth-success ${isModal ? "auth-card-modal auth-card-stepper" : ""}`}>
          {renderModalHeader()}
          <p className="eyebrow">Selesai</p>
          <h1>
            {submittedMode === "login"
              ? "Login siap dilanjutkan."
              : "Pendaftaran berhasil dilanjutkan."}
          </h1>
          <p>
            {submittedMode === "login"
        ...
        <div className={`auth-card glass ${isModal ? "auth-card-modal auth-card-stepper auth-card-intro" : ""}`}>
          ...
          <div className="auth-intro-content">
            ...
            <p className="auth-demo-hint">
              Demo mahasiswa: {demoCredentials.nim} / {demoCredentials.password}
            </p>
            <p className="auth-demo-hint">
              Demo admin/superadmin: {adminDemoCredentials.nim} / {adminDemoCredentials.password}
            </p>
            <div className="auth-intro-actions">
              <button
                type="button"
                className="button button-secondary auth-choice-button"
                onClick={() => {
                  setMode("login");
                  setCurrentStep(1);
                  setAuthActionError(null);
